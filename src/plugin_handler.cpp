...
namespace plgh
{
	...
	{
		bool load_success = LoadLibraryFunction(&ptr_driver_getDriverName, "driver_getDriverName", true)
			...
			&& LoadLibraryFunction(&ptr_driver_isConnected, "driver_isConnected", true)
			...
			&& LoadLibraryFunction(&ptr_driver_getFileSize, "driver_getFileSize", true)
			...
			&& LoadLibraryFunction(&ptr_driver_getlasterror, "driver_getlasterror", true);
		...
		if (load_success && !ptr_driver_isReadOnly())
		{
			...
			const bool load_write_funcs_success = LoadLibraryFunction(&ptr_driver_fwrite, "driver_fwrite", true)
				...
				&& LoadLibraryFunction(&ptr_driver_diskFreeSpace, "driver_diskFreeSpace", true)

				/* API functions definition, that can be defined optionally in the library only if the driver is not read-only */
				&& LoadLibraryFunction(&ptr_driver_copyToLocal, "driver_copyToLocal", false)
				&& LoadLibraryFunction(&ptr_driver_copyFromLocal, "driver_copyFromLocal", false);
