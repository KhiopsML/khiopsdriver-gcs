...
struct PluginHandle {
  ...
  long long (*ptr_driver_fread)(void *ptr, size_t size, size_t count,
                                void *stream);
  ...
  long long (*ptr_driver_fwrite)(const void *ptr, size_t size, size_t count,
                                 void *stream);
  ...
  /* API functions definition, that can be defined optionally in the library
   * only if the driver is not read-only */
...
private:
  ...
  handle_t LoadSharedLibrary(const std::string &library_name) {
    ...
    if (!handle) {
#ifdef __windows__
      std::cerr << "Failed loading module, error: " << std::hex
  ...
  bool LoadLibraryFunction(F_PTR *fptr, const char *function_name,
                           bool mandatory) {
